...
import webbrowser
from datetime import datetime  # Para pegar a data e hora atuais
...
class Mao:
    def __init__(self, hand_landmarks):
        ...
        self.anelar = Dedo(  # Adicionando o dedo anelar
            "Anelar",
    ...
    while cap.isOpened():
        ...
        if results.multi_hand_landmarks:
            for hand_landmarks in results.multi_hand_landmarks:
                ...
                # Verifica se o polegar está fechado
                if mao.polegar_fechado():
                    print("Polegar fechado. Funções desativadas.")
                    polegar_esta_fechado = True

                    if not mao.polegar_fechado() and mao.dedos_levantados()['indicador'] and mao.dedos_levantados()['medio'] and mao.dedos_levantados()['anelar']:
                        print("Gesto de mover mouse detectado")
                        
                        # Mover o mouse para a posição (182, 362) antes de criar o cartão
                        pyautogui.moveTo(182, 362)
                        sleep(1)  # Espera um pouco para garantir que o movimento foi registrado
                        pyautogui.press('n')  # Comando para abrir o novo cartão no Trello
                        print("Novo cartão aberto com a tecla N.")
                    
                    if not mao.dedos_levantados()["medio"] and all(mao.dedos_levantados()[dedo] for dedo in ["indicador", "anelar", "mindinho"]):
                        pyautogui.write(f"Nova atividade - {obter_data_hora()}")
                        pyautogui.press('enter')
                        print("Nova atividade escrita no terminal e Enter pressionado.")
                        sleep(1)

                else:
                    if polegar_esta_fechado:
                        print("Polegar voltou à posição normal. Funções reativadas.")
                    polegar_esta_fechado = False

                # Se o polegar estiver fechado, desativa os comandos
                if not polegar_esta_fechado:
                    # Movimento de pinça (polegar + indicador) para abrir o Trello
                    distancia_minima = 0.03
                    ...
                    if dist_polegar_indicador < distancia_minima:
                        print("Movimento de pinça com indicador detectado. Abrindo Trello.")
                        abrir_site("https://trello.com/b/XHNZaytY/teste-de-produtividade")
                        sleep(3)

                    # Movimento de pinça (polegar + médio) para fechar aba do navegador
                    ...
                    if dist_polegar_medio < distancia_minima:
                        print("Movimento de pinça com médio detectado. Fechando aba.")
